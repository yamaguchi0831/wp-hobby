...
## 2026-09-24 モバイル用ハンバーガーメニューの操作モックアップを作成

- 状態: 確認用モックアップ作成
- 実施内容: テーマ本体には変更を加えず、モバイル用ヘッダー、無料査定CTA、ハンバーガーボタン、右側ドロワーメニューの操作モックアップを作成した。指定された「買取方法」「買取品目一覧」「コラム」「よくある質問」「会社概要」を配置し、開閉、背景クリック、閉じるボタン、Escキー、メニュー選択の動作を実装した。
- 主な変更ファイル: `test/header-mobile-menu-mock.html`、`WORK-LOG.md`
- 未完了事項: ユーザーによるモックアップのデザイン・導線の確認。本体テーマへの実装。
- 次回の着手点: 承認後、`header.php`、共通CSS、共通JavaScriptへWordPressメニューを用いた実装として反映する。
...
## 2026-09-07 内部リンクカードブロック名を変更

- 状態: 完了
- 実施内容: エディターのブロック挿入画面で表示する名称を「ページ内リンクカード」から「サイト内リンクカード」へ変更した。ブロック識別子は変更していないため、既存の挿入済みブロックへの影響はない。
- 主な変更ファイル: `theme/buybuycoms-hobby/asset/js/blocks/internal-link-card.js`、`WORK-LOG.md`
- 未完了事項: WordPress管理画面での表示確認。
- 次回の着手点: ブロック挿入画面で「サイト内リンクカード」と表示されることを確認する。

## 2026-09-07 ファビコンをHBロゴへ更新

- 状態: 完了
- 実施内容: 指定された`favicon.png`を元に、16・32・48・64・128・192・256pxを収録するマルチサイズICOへ変換し、既存の`favicon.ico`を置き換えた。共通ヘッダーは同ファイルを参照済みのため、追加のテンプレート変更は不要。
...
## 2026-09-07 サイト内リンクカードへコラム説明文を追加

- 状態: 完了
- 実施内容: サイト内リンクカードで`column`を選択した場合だけ、投稿に個別登録されたAll in One SEOのメタディスクリプションをタイトル下へ出力するよう追加した。AIOSEOのAPIで取得し、未稼働時・旧データ形式では既知のメタキーを参照する。説明文はHTMLを除去して空白を整形し、50文字を超える場合は末尾を`…`で省略する。
...
# 2026-09-09 コラム用買取方法カードの見出し順を変更

- 状態: 完了
- 実施内容: カスタムブロック「買取方法（コラム用）」で使用する`column-auto-tabs`バリエーションに限り、各カードの見出し（`h3`）をイメージ画像より先に出力するよう変更した。通常ページ用および確認ページの`column-tabs`バリエーションのカード順には影響しない。
- 主な変更ファイル: `theme/buybuycoms-hobby/template-parts/common/purchase-methods.php`、`WORK-LOG.md`
- 未完了事項: WordPressのブロックエディターと公開画面におけるPC・モバイル表示確認。
- 次回の着手点: コラム本文へブロックを挿入し、各カードで見出しの直下に画像が表示されることを確認する。

# 2026-09-09 コラム用買取方法のおすすめバッジ重なりを修正

- 状態: 完了
- 実施内容: コラム用カスタムブロックの3列カード表示時だけ、宅配買取の「おすすめ」バッジを画像領域内の下端へ配置した。見出しを画像より前へ移動したことで発生した説明文との重なりを解消し、1列タブ表示となるスマホ幅の配置は変更していない。
- 主な変更ファイル: `theme/buybuycoms-hobby/asset/css/component.css`、`WORK-LOG.md`
- 未完了事項: WordPressのブロックエディターと公開画面における表示確認。
- 次回の着手点: PC幅とスマホ幅で、宅配買取カードのバッジと本文が重ならないことを確認する。

# 2026-09-09 コラム用買取方法のおすすめバッジ位置を微調整

- 状態: 完了
- 実施内容: PC幅のコラム用買取方法カードにおける「おすすめ」バッジの下位置を`--hb-space-xs`から`--hb-space-2xs`へ変更した。
- 主な変更ファイル: `theme/buybuycoms-hobby/asset/css/component.css`、`WORK-LOG.md`
- 未完了事項: WordPressのブロックエディターと公開画面における表示確認。
...
# 2026-09-10 プロジェクト現況確認

- 状態: 確認完了
- 実施内容: 静的参照データ、WordPressテーマ本体、テーマ化計画、直近の作業ログ、Git状態を確認した。テーマ本体には主要テンプレートおよび責務別の `inc/` 実装があり、作業ツリーの未追跡ファイルは配布用と見られる `theme/buybuycoms-hobby.zip` のみだった。
- 主な確認ファイル: `WP-THEME-CONVERSION-PLAN.md`、`WORK-LOG.md`、`theme/buybuycoms-hobby/`
- 未完了事項: WordPress実行環境でのレスポンシブ・ブロックエディター・フォーム送信・空データ・本番キャッシュを含む表示検証。
- 次回の着手点: 直近のUI変更（コラム用買取方法カード、ジャンルヒーローバッジ）をPC・モバイルの実画面で確認する。

# 2026-09-10 フッターの買取品目をgenreターム一覧へ変更

- 状態: 完了
- 実施内容: フッターの固定4リンクを廃止し、カスタムタクソノミー`genre`の全タームを取得して一覧表示するよう変更した。既存の`genre-order`メタによる並び順を再利用し、各項目は対応するタームアーカイブへリンクする。タクソノミーが未登録またはターム未登録の場合は、固定リンクを代替表示せず当該列を出力しない。
- 主な変更ファイル: `theme/buybuycoms-hobby/footer.php`、`WORK-LOG.md`
- 検証: XAMPP同梱PHPによる`footer.php`の構文検証、およびGit差分の空白エラー確認を実施。
- 未完了事項: WordPress実行環境での実ターム数に応じたPC・モバイルのフッターレイアウト確認。
- 次回の着手点: `genre`の登録・並び順を管理画面で確認し、各リンク先とフッターの折り返し表示を確認する。

# 2026-09-11 プロジェクト現況確認

- 状態: 確認完了
- 実施内容: WordPressテーマの構成、進捗資料、直近の作業ログ、未完了マーカー、Git状態を確認した。テーマはクラシックテーマとして主要テンプレート、テンプレートパーツ、責務別の`inc/`、CSS・JavaScript・画像アセットを備える。未追跡ファイルは配布用の`theme/buybuycoms-hobby.zip`のみ。
- 主な確認ファイル: `theme/buybuycoms-hobby/`、`theme/buybuycoms-hobby/PHASE-1-STATUS.md`、`theme/buybuycoms-hobby/README.md`、`THEME-COMPLETION-CHECKLIST.md`
- 未完了事項: WordPress実行環境におけるPC・モバイル・エディター・空データ・境界値・フォーム配送・本番キャッシュの検証、およびCPT／タクソノミー／ACFの登録主体と運用手順の文書化。
- 次回の着手点: 直近のUI変更とフッターのgenreターム一覧を、WordPress実行環境でPC・モバイル表示およびリンク先を確認する。

# 2026-09-11 フッターリンクリストの2段レイアウト化

- 状態: 完了
- 実施内容: フッターのリンク領域を2段構成へ変更した。上段は`genre`タームを横並び・折り返しで表示する「買取品目」とし、下段は左から「サービス」「サポート」「会社情報」の3列に配置した。画面幅980px以下ではブランド情報とリンク領域を縦並びにし、640px以下では下段リンクを2列へ再配置する。タクソノミーまたはタームが未登録の場合は従来どおり買取品目欄を出力しない。
- 主な変更ファイル: `theme/buybuycoms-hobby/footer.php`、`theme/buybuycoms-hobby/asset/css/component.css`、`WORK-LOG.md`
- 検証: XAMPP同梱PHPで`footer.php`の構文検証、Git差分の空白エラー確認を実施。
- 未完了事項: WordPress実行環境で実ターム数に応じたPC・モバイルの折り返しと、フッターのリンク先を確認する。
- 次回の着手点: `genre`タームが多数ある状態で、上段の横並びと下段3列の表示をPC・モバイルで確認する。

# 2026-09-11 フッターサービスリンクの追加

- 状態: 完了
- 実施内容: フッター下段の「サービス」リンクリストへ、固定ページ`/reason/`の「選ばれる理由」と、コラムアーカイブ`/column/`の「コラム」を追加した。
- 主な変更ファイル: `theme/buybuycoms-hobby/footer.php`、`WORK-LOG.md`
...
# 2026-09-14 買取事例詳細の商品画像を全体表示へ変更

- 状態: 完了
- 実施内容: 買取事例詳細ページの商品画像を、既存の画像領域内で縦横比を維持したまま全体表示するよう変更した。画像領域に収まらない画像は`object-fit: contain`で縮小し、トリミングしない。
- 主な変更ファイル: `theme/buybuycoms-hobby/asset/css/page-static.css`、`WORK-LOG.md`
- 検証: 対象セレクタがPC・モバイル双方で同じ`object-fit: contain`を使用すること、およびGit差分の空白エラーがないことを確認。
- 未完了事項: WordPress実行環境で縦長・横長の商品写真の表示確認。
- 次回の着手点: 買取事例詳細ページをPC・モバイルで開き、写真全体が画像領域内に収まることを確認する。

# 2026-09-14 ACF設定エクスポートの保管

- 状態: 完了
- 実施内容: ACF管理画面から書き出した一括エクスポートJSONをプロジェクト内へ原本のまま保存した。フィールドグループ5件、タクソノミー2件、カスタム投稿タイプ4件を確認し、復元方法とLocal JSONとの違いをREADMEへ記録した。
- 主な変更ファイル: `config/acf-export/acf-export-2026-09-14.json`、`config/acf-export/README.md`、`WORK-LOG.md`
...
# 2026-09-15 コラム用「買取実績」カスタムブロックを追加

- 状態: 実装完了
- 実施内容: `column`本文で使用する動的ブロック「買取実績（コラム用）」を追加した。設置コラムに紐づく`genre`と一致する公開済み`purchase-record`を新着順で最大6件表示する。個別指定した実績を先頭に表示し、6件未満の場合は重複を除いて新着実績で補完する。表示対象がない場合は「実績がありません」と出力する。編集画面にはREST APIを利用した検索付きプルダウンを設け、候補にタイトルと`item-price`を表示し、最大6件まで指定・削除できるようにした。カードは既存の買取実績テンプレートパーツを再利用し、このブロックに限りPC3列・スマホ2列で表示する。
- 主な変更ファイル: `theme/buybuycoms-hobby/inc/blocks.php`、`theme/buybuycoms-hobby/asset/js/blocks/column-purchase-records.js`、`theme/buybuycoms-hobby/template-parts/common/purchase-records.php`、`theme/buybuycoms-hobby/asset/css/component.css`、`theme/buybuycoms-hobby/README.md`、`WORK-LOG.md`
- 検証: 変更したPHPファイルの構文検査、ブロックJavaScriptの構文検査、Git差分の空白エラー確認を実施した。選択IDは公開済み買取実績に限定し、重複除外・最大6件・表示順保持・空状態をコード上で確認した。
- 未完了事項: WordPressブロックエディターでの検索・追加・削除・保存・再編集、`genre`未設定／1件／複数件、買取実績0件／6件未満／6件以上、個別指定0〜6件の実データ検証。PC・スマホの実表示確認。保管中のACFエクスポートでは`genre`の対象に`purchase-record`が設定され、`column`投稿タイプ側のタクソノミーにも`genre`が設定されていることを確認済みだが、別環境へ復元後の関連付けは未確認。
- 次回の着手点: `genre`が設定されたコラムへブロックを挿入し、自動6件、個別指定と新着補完、空状態、PC3列・スマホ2列を順に確認する。

# 2026-09-15 コラム用買取実績ブロックのエディター内ボタン表示を修正

- 状態: 完了
- 実施内容: ブロックエディターではフロント用リセットCSSを読み込まないため、買取実績カードのボタンがcontent-box扱いとなり、`width: 100%`にパディングが加算されてカード外へはみ出していた。カードボタンへ`box-sizing: border-box`を追加し、エディター共通リンクスタイルに上書きされていたボタンの文字色と下線もエディター内で補正した。
- 主な変更ファイル: `theme/buybuycoms-hobby/asset/css/component.css`、`WORK-LOG.md`
- 検証: 対象セレクタの詳細度とCSS読込構成を確認し、Git差分に空白エラーがないことを確認した。
- 未完了事項: WordPressブロックエディターでの修正後の目視確認。
- 次回の着手点: コラム編集画面を再読み込みし、各カードの「査定を申し込む」ボタンがカード幅に収まり、白文字・下線なしで表示されることを確認する。

# 2026-09-15 FAQ専用ページの無料条件に関する注意書きを追加

- 状態: 完了
- 実施内容: よくあるご質問ページの質問「本当に無料なの？」の既存回答後に改行を加え、「※ただし、査定合計5,000円未満時、送料及び返送料は無料対象外となりますのでご注意ください。」を追記した。静的HTMLとWordPressテンプレートを同時に更新した。
- 主な変更ファイル: `pages/page-faq.html`、`theme/buybuycoms-hobby/page-faq.php`、`WORK-LOG.md`
- 検証: 対象回答の直後へ改行と注意書きが反映されていること、`page-faq.php`にPHP構文エラーがないこと、Git差分に空白エラーがないことを確認した。
- 未完了事項: WordPress実行環境でのFAQ展開表示確認。
- 次回の着手点: よくあるご質問ページで「本当に無料なの？」を展開し、注意書きが既存回答の次行に表示されることを確認する。

# 2026-09-15 FAQ専用ページの送料条件に関する注意書きを追加

- 状態: 完了
- 実施内容: よくあるご質問ページの質問「本当に送料は無料なの？」の既存回答後に改行を加え、「※ただし、査定合計5,000円未満時、送料及び返送料は無料対象外となりますのでご注意ください。」を追記した。静的HTMLとWordPressテンプレートを同時に更新した。
- 主な変更ファイル: `pages/page-faq.html`、`theme/buybuycoms-hobby/page-faq.php`、`WORK-LOG.md`
- 検証: 対象回答の直後へ改行と注意書きが反映されていること、`page-faq.php`にPHP構文エラーがないこと、Git差分に空白エラーがないことを確認した。
- 未完了事項: WordPress実行環境でのFAQ展開表示確認。
- 次回の着手点: よくあるご質問ページで「本当に送料は無料なの？」を展開し、注意書きが既存回答の次行に表示されることを確認する。

# 2026-09-15 コラム用買取実績ブロックの見出し階層を修正

- 状態: 完了
- 実施内容: コラム用「買取実績」ブロックへ、設置コラムの先頭`genre`名を使用した`h2`見出し「○○の買取価格表」を追加した。`genre`未設定時は「買取価格表」と表示する。ブロック内の各カードタイトルは`h3`から`p`へ変更し、既存classを維持して見た目を変えないようにした。共有テンプレートパーツにはタイトルタグ指定を追加し、コラム用ブロック以外の買取実績カードは従来どおり`h3`を使用する。
- 主な変更ファイル: `theme/buybuycoms-hobby/inc/blocks.php`、`theme/buybuycoms-hobby/template-parts/common/purchase-records.php`、`theme/buybuycoms-hobby/asset/css/component.css`、`theme/buybuycoms-hobby/README.md`、`WORK-LOG.md`
- 検証: コラム用ブロックが`card_title_tag`へ`p`を渡し、共有テンプレートのデフォルトが`h3`のままであることをコード上で確認した。変更したPHPの構文検査とGit差分の空白エラー確認を実施した。
- 未完了事項: WordPressブロックエディターと公開画面での`genre`名入り見出し、ジャンル未設定時の見出し、カードタイトルのHTML・スタイル確認。
- 次回の着手点: `genre`設定済み／未設定のコラムでブロックを表示し、見出し文言とカードタイトル要素を確認する。

# 2026-09-16 プロジェクト現状確認

- 状態: 調査完了
- 実施内容: プロジェクト構成、Git状態、デザインガイド、デザイントークン、WordPressテーマ変換計画、テーマREADME、第1段階検証状況、完成チェックリスト、主要テーマ基盤、未コミット差分を確認した。テーマは第1段階のPHPテンプレート化と主要データ連携まで実装済みで、実環境・境界値検証と運用仕様の確定が主な残作業であることを整理した。
- 主な変更ファイル: `WORK-LOG.md`（調査記録のみ）
- 検証: テーマ内PHP 48ファイルの構文検査、JavaScript 12ファイルの構文検査、`git diff --check`を実施し、すべて問題なし。`wp_head()`、`wp_body_open()`、`wp_footer()`、`language_attributes()`、`body_class()`の実装と、WordPress側に`include-components.js`依存がないことを確認した。
- 未完了事項: WordPress実行環境でのPC・モバイル表示、操作、メール、空データ・境界値、エディター表示の検証。CPT・タクソノミー・ACFの登録主体、対応バージョン、公開・ロールバック手順等の運用仕様確定。`screenshot.png`、`theme.json`、`comments.php`、`searchform.php`は未配置で、採用要否の判断が必要。`placehold.co`の仮画像と暫定リンクが残る。
- 次回の着手点: コラム用「買取実績」ブロックの実データ検証を行い、その後に主要画面のPC・モバイル表示、404・コンソール・`WP_DEBUG`、フォーム送信を順に確認する。

# 2026-09-16 宅配買取フォームの段ボール重量注記を太字化

- 状態: 完了
- 実施内容: 宅配買取で買取キットを選択した際に表示される段ボールリストについて、S・M・L・LL各サイズの「※内容量は○kg未満」という重量注記を`strong`要素へ変更し、寸法表記と区別して太字表示されるようにした。静的HTMLとWordPressテンプレートを同時に更新した。
- 主な変更ファイル: `pages/page-contact.html`、`theme/buybuycoms-hobby/page-contact.php`、`WORK-LOG.md`
...
# 2026-09-24 コラム用買取方法ブロックの見出し削除

- 状態: 完了
- 実施内容: カスタムブロック「買取方法（コラム用）」の表示から「あなたに合わせて選べる、3つの買取方法」の見出しを削除した。共通テンプレートへ見出し表示の切り替えを追加し、ブロック経由の出力だけで見出しを非表示にしているため、確認ページなど他の利用箇所の見出しは維持される。
...
# 2026-09-25 モバイルメニューモックのスタンドアロン化

- 状態: 完了
- 実施内容: クライアント確認用のモバイルメニューモックを、HTMLファイル単体で表示できるスタンドアロン版として生成した。デザイントークンCSSをHTML内へ展開し、ロゴ画像をPNGのData URIとして埋め込んだ。元のモックは変更せず、再生成用PowerShellスクリプトを追加した。
- 主な変更ファイル: `test/header-mobile-menu-mock-standalone.html`、`tools/build-header-mobile-menu-standalone.ps1`、`WORK-LOG.md`
- 検証: スタンドアロン版に親ディレクトリを参照するCSS・画像URLが残っていないこと、トークンCSSとロゴ画像が埋め込まれていること、390px幅のブラウザ上でロゴ表示とメニューの開閉が動作すること、コンソール警告・エラーがないことを確認した。
- 未完了事項: クライアント側の利用ブラウザでの表示確認。
- 次回の着手点: 元モックのCSSまたはロゴを更新した場合は、`tools/build-header-mobile-menu-standalone.ps1`を再実行して配布用HTMLを更新する。

# 2026-09-25 モバイルメニューモック内CTAの配置調整

- 状態: 完了
- 実施内容: `header-mobile-menu-mock.html`のメニュー内CTAが画面下端に寄りすぎないよう、ヘッダーとナビゲーションの下に残る余白領域の中央へ配置するグリッド構成へ変更した。クライアント送付用のスタンドアロン版は変更していない。
- 主な変更ファイル: `test/header-mobile-menu-mock.html`、`WORK-LOG.md`
- 検証: 390×844pxと375×667pxでメニューを開き、CTAの中心がナビゲーション下の余白領域の中心と一致すること、短い画面でもCTAがドロワー内に収まること、メニューの開閉が維持されること、コンソール警告・エラーがないことを確認した。
- 未完了事項: クライアント側の利用端末での表示確認。
- 次回の着手点: 必要に応じて実機の画面高に合わせてCTA位置を微調整する。

# 2026-09-25 モバイルヘッダーメニューをWordPressテーマへ反映

- 状態: 実装完了
- 実施内容: クライアント確認済みのモバイルヘッダーと右側ドロワーメニューをWordPressテーマへ移植した。既存の`primary`メニューをPC・モバイルで共用しつつ、モバイル側のメニュー項目IDを省略して重複IDを防止した。無料査定CTAはナビゲーション下の残り余白の縦中央に配置し、背景タップ、閉じるボタン、Escキー、リンク選択、PC幅への切り替えで閉じる処理、フォーカス復帰、背景スクロール抑止を追加した。
- 主な変更ファイル: `theme/buybuycoms-hobby/header.php`、`theme/buybuycoms-hobby/inc/template-functions.php`、`theme/buybuycoms-hobby/asset/css/component.css`、`theme/buybuycoms-hobby/asset/js/component.js`、`WORK-LOG.md`
- 検証: PHP構文とGit差分の空白エラーを確認した。テーマ本体のCSS・JavaScriptを読み込む確認ページを使い、390×844pxでCTAが残り余白の中央に配置されること、320×667pxでヘッダーが横にはみ出さないこと、背景タップとEscキーで閉じること、`aria-expanded`・`aria-hidden`・`inert`・bodyスクロール固定が正しく切り替わること、コンソール警告・エラーがないことを確認した。
- `THEME-COMPLETION-CHECKLIST.md`: 今回に関係する共通ヘッダー、WordPressメニュー、外部JavaScript化、モバイル表示、キーボード操作、フォーカス制御、ID重複防止を確認。テーマ全体の完成チェックは未実施。
- 未完了事項: 実際のWordPress環境で管理画面から割り当てた`primary`メニューの内容と実機表示を確認する必要がある。
- 次回の着手点: WordPress環境でログイン・未ログインの両方を確認し、管理バー表示時を含むモバイルドロワーの高さとリンク遷移を実機確認する。

# 2026-09-25 モバイルメニュー確認ファイルの整理

- 状態: 完了
- 実施内容: テーマへの反映完了後、クライアント確認用の`test`フォルダを削除した。削除済みHTMLを参照するスタンドアロン生成スクリプトも不要になったため削除し、テーマ本体だけで完結する状態に整理した。
- 主な変更ファイル: `test/header-mobile-menu-mock.html`（削除）、`tools/build-header-mobile-menu-standalone.ps1`（削除）、`WORK-LOG.md`
- 検証: Git差分を確認し、テーマ実装に必要なファイルが削除対象へ含まれていないことを確認した。
- 未完了事項: なし。
- 次回の着手点: 実際のWordPress環境でモバイルヘッダーメニューを最終確認する。
